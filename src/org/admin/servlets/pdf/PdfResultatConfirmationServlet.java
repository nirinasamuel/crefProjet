...
@WebServlet("/pdf_resultat_confirmation")
public class PdfResultatConfirmationServlet extends BaseServlet {
	private static final long serialVersionUID = 1L;
    private CandidatDao candidatDao;
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PdfResultatConfirmationServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else
		{
			...
			try
			{
				...
				List<Candidat> candidats = candidatDao.getAllCandidatConfirmed(choix);
	...
	public void createPdf(String dest, List<Candidat> candidats) throws IOException, DocumentException {
        ...
        Paragraph titre=new Paragraph("    Liste des étudiants confirmés en L1 - "+candidats.get(0).getChoix(),font2);
        /* String textList="";
        if(id_vague==1)
        ...
        */
        ...
        for(int i = 0; i < candidats.size() ; i++){
			Paragraph data = new Paragraph(candidats.get(i).getNom()+" "+candidats.get(i).getPrenoms(),font);
			...
			Paragraph recu = new Paragraph(candidats.get(i).getNum_recu(),font);
			...
			*/
			PdfPCell cell_recu = new PdfPCell();
            cell_recu.addElement(recu);    
			table.addCell(cell_recu);
